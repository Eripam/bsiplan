...
module.exports.Login = async function (req, callback) {
  try {
    const response = await pool.pool.query("select * from (select * from seguridad.vlogin where rpe_persona like '"+req.body.perid+"' limit 1)as con inner join seguridad.rol_opcion on rpe_rol=rop_rol join seguridad.opciones on rop_opcion=opc_codigo order by rop_padreop, rop_opcion limit 1;");
...
module.exports.LoginCorreo = async function (req, callback) {
  try {
    const response = await pool.pool.query("select * from (select * from seguridad.vlogin where per_email like '"+req.body.perid+"' limit 1)as con inner join seguridad.rol_opcion on rpe_rol=rop_rol join seguridad.opciones on rop_opcion=opc_codigo order by rop_padreop, rop_opcion limit 1;");
    if (response.rowCount > 0) {
      callback(true, response.rows);
    } else {
      callback(false);
...
//Eta registrado ese usuario para acceder a la pagina
