...
//Lista roles por persona y dependencia que se encuentren registrados en el sistema
module.exports.RolesPersonaDep = async function (req, callback) {
  try {
    const response = await pool.pool.query(
      "select *, CASE when rpe_estado=1 then 'Activo' when rpe_estado=0 then 'Inactivo' end rpe_estado_nombre from seguridad.rol_persona inner join seguridad.persona on rpe_persona=per_codigo inner join seguridad.dependencia on rpe_dependencia=dep_codigo inner join seguridad.rol on rpe_rol=rol_codigo where per_estado=1 and rol_estado=1 and dep_estado=1 and (rpe_dependencia='"+req.body.codigo+"' or dep_codcodigo='"+req.body.codigo+"') order by rpe_persona, dep_codigo, rol_codigo, rpe_estado;"
    );
    if (response.rowCount > 0) {
      callback(true, response.rows);
    } else {
