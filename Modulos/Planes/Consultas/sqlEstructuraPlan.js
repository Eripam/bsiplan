...
module.exports.ListarEstructuraPlan = async function (req, callback){
    try {
      var response;
        if(req.body.tipo==1){
          response = await pool.pool.query("SELECT ep.eplan_id, ep.eplan_nombre, ep.eplan_plan, ep.eplan_estructura, ep.eplan_eplan_id, ep.eplan_estado, est.est_id, est.est_nombre, (est.est_codigo || '-'|| ep.eplan_codigo) as codigo, (est2.est_codigo || '-'||ep2.eplan_codigo) as codigopadre, est.est_orden, case when ep.eplan_estado=1 then 'Activo' when ep.eplan_estado=0 then 'Inactivo' end as estadonombre, (select count(eplan_id) from planes.estructura_planes where eplan_eplan_id=ep.eplan_id) FROM planes.estructura_planes as ep inner join planes.estructura as est on ep.eplan_estructura=est_id left join planes.estructura_planes as ep2 on ep.eplan_eplan_id=ep2.eplan_id left join planes.estructura as est2 on ep2.eplan_estructura=est2.est_id where ep.eplan_plan='"+req.body.codigo+"' and ep.eplan_estructura='"+req.body.estructura+"' order by ep.eplan_estructura, ep.eplan_codigo;");
          ...
          response = await pool.pool.query("SELECT ep.eplan_id, ep.eplan_codigo, ep.eplan_nombre, est.est_codigo FROM planes.estructura_planes as ep inner join planes.estructura as est on ep.eplan_estructura=est_id where ep.eplan_plan='"+req.body.codigo+"' and ep.eplan_estado=1 and (est.est_orden='"+orden+"' or est.est_orden='"+req.body.orden+"') order by ep.eplan_estructura, ep.eplan_codigo;");
        }else{
    ...
    try {
        var response;
          if(req.body.eplan_eplan_id==0){
                  response = await pool.pool.query("INSERT INTO planes.estructura_planes(eplan_id, eplan_codigo, eplan_nombre, eplan_plan, eplan_estructura) VALUES ((select * from planes.f_codigo_planes(3)), (select * from planes.f_codigo_estructuraplan('"+req.body.eplan_estructura+"')), '"+req.body.eplan_nombre+"', '"+req.body.eplan_plan+"', '"+req.body.eplan_estructura+"');");
          }else{
                  response = await pool.pool.query("INSERT INTO planes.estructura_planes(eplan_id, eplan_codigo, eplan_nombre, eplan_plan, eplan_estructura, eplan_eplan_id) VALUES ((select * from planes.f_codigo_planes(3)), (select * from planes.f_codigo_estructuraplan('"+req.body.eplan_estructura+"')), '"+req.body.eplan_nombre+"', '"+req.body.eplan_plan+"', '"+req.body.eplan_estructura+"', '"+req.body.eplan_eplan_id+"');");
          }
        if (response.rowCount > 0) {
            callback(true);
          } else {
